    ...
    function _mltlngg_plugin_activate() {
        ...
        if (isset($mltlngg_options['deactivation'])) {
            ...
            foreach ($posts as $key => $value) {
                $wpdb->update(
                        $wpdb->prefix . 'mltlngg_translate', array(
...
if (!function_exists('mltlngg_plugin_db')) {
    ...
    function mltlngg_plugin_db() {
        ...
        $mltlngg_sql = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "mltlngg_translate` (
        ...
        $mltlngg_sql = "CREATE TABLE IF NOT EXISTS `" . $wpdb->prefix . "mltlngg_terms_translate` (
			`ID` INT(6) UNSIGNED NOT NULL AUTO_INCREMENT,
...
if (!function_exists('mltlngg_alternate_links')) {
    ...
    function mltlngg_alternate_links() {
        ...
        foreach ($link_attr_args as $item) {
            ...
            printf(
                    '<link rel="alternate" hreflang="%1$s" href="%2$s" />', strtolower(str_replace('_', '-', esc_attr($item['hreflang']))), esc_url(rtrim($language_link, '/'))
            );
        }
    }
...
if (!function_exists('mltlngg_script_style')) {
    ...
    function mltlngg_script_style() {
        ...
        if (is_admin()) {
            ...
            wp_localize_script('mltlngg_script', 'mltlngg_vars', array(
...
if (!class_exists('Mltlngg_Widget')) {
    ...
    class Mltlngg_Widget extends WP_Widget {
        ...
        function __construct() {
            parent::__construct(
                    'multi_language_widget', /* Base ID */ 'Multilanguage', /* Name */ array('description' => __('Content language switch', 'multilanguage'),) /* Args */
            );
        }
        ...
        public function form($instance) {
            ...
            <div style="clear: both;">
                <label class="mltlngg_widget_label">
                    <input id="<?php echo $this->get_field_id('first-letters'); ?>" name="<?php echo $this->get_field_name('mltlngg_language_switcher'); ?>" type="radio" value="first-letters" <?php checked($mltlngg_language_switcher, 'first-letters'); ?> /><?php _e('Letras', 'multilanguage'); ?>
                </label>
            </div>
    ...
    function mltlngg_get_switcher_block($mltlngg_language_switcher = false) {
        ...
        switch ($mltlngg_language_switcher) {
            case 'drop-down-list':
                ...
                $switcher .= '<ul class="mltlngg-lang-switch mltlngg-lang-switch-names">
						<li>
							<a>
            ...
            case 'drop-down-icons':
                ...
                $switcher .= '<ul class="mltlngg-lang-switch">
						<li>
            ...
            case 'flags-icons':
                foreach ($mltlngg_enabled_languages as $item) {
                    $flag = ( empty($item['flag']) ) ? plugins_url('images/flags/', __FILE__) . $item['locale'] . '.png' : $item['flag'];
                    $switcher .= '<button class="mltlngg-lang-button-icons" name="mltlngg_change_display_lang" value="' . $item['locale'] . '" title="' . $item['name'] . '">
            ...
            case 'first-letters':
                ...
                foreach ($mltlngg_enabled_languages as $item) {
                    $cont ++;
                    $flag = ( empty($item['flag']) ) ? plugins_url('images/flags/', __FILE__) . $item['locale'] . '.png' : $item['flag'];
                    $switcher .= "<span class='language-letters'>";
                    $switcher .= '<button class="mltlngg-lang-button-icons" name="mltlngg_change_display_lang" value="' . $item['locale'] . '" title="' . $item['name'] . '">';
                    $switcher .= substr($item['locale'], 0, 2);
                    $switcher .= '</button>';
                    if ($cont < count($mltlngg_enabled_languages)) $switcher .= "|";
                    $switcher .= "</span>";
                }
                break;
        }
...
if (!function_exists('mltlngg_settings_page')) {
    ...
    function mltlngg_settings_page() {
        ...
        <?php
        if ('mltlngg_settings' == $_GET['page']) { /* Showing settings tab */
    ...
    function mltlngg_new_terms_created($term_id) {
        ...
        foreach ($mltlngg_enabled_languages as $one_language) {
            ...
            if (isset($_POST[$cat_lang]) && "" != $_POST[$cat_lang]) { /* If translation is empty, do not create translation in database */
                $wpdb->insert(
                        $wpdb->prefix . 'mltlngg_terms_translate', array(
                    'term_ID' => $term_id,
                    'name' => $_POST[$cat_lang],
...
if (!function_exists('mltlngg_search_where')) {
    ...
    function mltlngg_search_where($where) {
        ...
        if (!is_admin() && is_main_query() && is_search()) {
            if ('all' == $mltlngg_options['search']) {
                $where = preg_replace(
                        "/\($wpdb->posts.post_title\s+LIKE\s*(\'[^\']+\')\s*\)/", "(post_title LIKE $1) OR (mltlngg_post_title LIKE $1) OR (mltlngg_post_content LIKE $1) OR (mltlngg_post_excerpt LIKE $1)", $where
                );
            } else if ($mltlngg_get_default_language != $mltlngg_current_language) {
                $where = preg_replace(
                        "/\($wpdb->posts.post_title\s+LIKE\s*(\'[^\']+\')\s*\)/", "(mltlngg_post_title LIKE $1)", $where
                );
                $where = preg_replace(
                        "/\($wpdb->posts.post_content\s+LIKE\s*(\'[^\']+\')\s*\)/", "(mltlngg_post_content LIKE $1)", $where
                );
                $where = preg_replace(
                        "/\($wpdb->posts.post_excerpt\s+LIKE\s*(\'[^\']+\')\s*\)/", "(mltlngg_post_excerpt LIKE $1)", $where
                );
            }
        }
        return $where;
